    ...
    Scheme::ParamsProver: Send + Sync,
{
    ...
    let s = FxBuildHasher;
    let mut challenges = HashMap::<usize, Scheme::Scalar>::with_capacity_and_hasher(cs.num_challenges(), s);
    ...
    let res = Ok(prover.create_proof()?);

    // TODO - do this even count correctly?
    #[cfg(feature = "counter")]
    {
        use crate::{FFT_COUNTER, MSM_COUNTER};
        use std::collections::BTreeMap;
