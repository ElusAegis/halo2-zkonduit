        ...
        ConstraintSystemMid {
            ...
            lookups: cs
                ...
                .map(|l| halo2_middleware::lookup::ArgumentMid {
                    ...
                    inputs_expressions: l.inputs_expressions.into_iter().map(|e| e.iter().map(|e| e.clone().into()).collect()).collect(),
...
/// TODO - consider if we can remove it
#[derive(Debug, Clone)]
...
pub struct ConstraintSystem<F: Field> {
    ...
    #[cfg(feature = "mv-lookup")]
    pub lookups_map: BTreeMap<String, lookup::Argument<F>>, // TODO - confirm if we want to go away from Lookup Tracker

    // Vector of lookup arguments, where each corresponds to a sequence of
    // input expressions and a sequence of table expressions involved in the lookup.
    // TODO - check if we want to make this conditional
...
impl<F: Field> ConstraintSystem<F> {
    ...
    pub fn lookup<S: AsRef<str>>(
        ...
        self.lookups_map
            ...
            .and_modify(|table_tracker| table_tracker.inputs_expressions.push(input_expressions.clone()))
            .or_insert(lookup::Argument::new(name.as_ref(), &table_expressions, &vec![input_expressions]));
    }
    ...
    pub fn chunk_lookups(mut self) -> Self {
        ...
        let max_single_lookup_degree: usize = self
            ...
            .values()
            .map(|v| {
                let table_degree = v.table_expressions.iter().map(|expr| expr.degree()).max().unwrap();
        ...
        for v in self.lookups_map.values() {
            let lookup::Argument { name, table_expressions, inputs_expressions } = v;

            let mut count = 0;
            let mut args = vec![lookup::Argument::new(format!("{name}_{count}"), table_expressions, &[inputs_expressions[0].clone()])];

            for input in inputs_expressions.iter().skip(1) {
                ...
                if !indicator {
                    count += 1;
                    args.push(lookup::Argument::new(format!("{name}_{count}"), table_expressions, &[input.clone()]))
                }
            }
    ...
    pub fn lookup_any<S: AsRef<str>>(
        ...
        self.lookups_map
            .entry(table_expressions_identifier.clone())
            .and_modify(|table_tracker| table_tracker.inputs_expressions.push(input_expressions.clone()))
            .or_insert(lookup::Argument::new (
                name,
                &table_expressions,
                &vec![input_expressions],
            ));
    }
