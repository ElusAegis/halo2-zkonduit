...
use crate::{
    ...
    multicore::{IntoParallelRefIterator, ParallelIterator, IndexedParallelIterator},
    poly::{Coeff, ExtendedLagrangeCoeff, Polynomial},
};

use halo2_middleware::circuit::Any;
use halo2_middleware::poly::Rotation;
use halo2_middleware::ff::{Field, PrimeField, WithSmallOrderMulGroup};

use super::shuffle;

#[cfg(feature = "mv-lookup")]
use halo2_middleware::ff::BatchInvert;
...
pub(crate) struct Evaluator<C: CurveAffine> {
    ...
    ///  Lookups evalution
    #[cfg(feature = "mv-lookup")]
    lookups: Vec<(Vec<GraphEvaluator<C>>, GraphEvaluator<C>)>,
    #[cfg(not(feature = "mv-lookup"))]
...
impl<C: CurveAffine> Evaluator<C> {
    ...
    pub fn new(cs: &ConstraintSystemBack<C::ScalarExt>) -> Self {
        ...
        for lookup in cs.lookups.iter() {
            ...
            let evaluate_lc = |graph: &mut GraphEvaluator<C>, expressions: &Vec<ExpressionBack<_>>| {
    ...
    pub(in crate::plonk) fn evaluate_h(
            ...
            .zip(permutations.iter())
        {
            ...
            let inputs_inv_sum_cosets: Vec<_> = lookups
                ...
                .enumerate()
                .map(|(n, lookup)| {
                    let (inputs_lookup_evaluator , _):  &(Vec<GraphEvaluator<_>>, _) = &self.lookups[n];
            ...
            for (n, lookup) in lookups.iter().enumerate() {
                ...
                // Lookup constraints
                parallelize(&mut values, |values, start| {
                    let lookup_evaluator : &GraphEvaluator<_> = &self.lookups[n];
            ...
            for (n, shuffle) in shuffles.iter().enumerate() {
                ...
                // Shuffle constraints
                parallelize(&mut values, |values, start| {
                    let input_evaluator : &GraphEvaluator<_>  = &self.shuffles[2 * n];
                    let shuffle_evaluator : &GraphEvaluator<_> = &self.shuffles[2 * n + 1];
