...
use crate::plonk::lookup::{self, prover::lookup_commit_permuted};
use crate::plonk::{ permutation, permutation::prover::permutation_commit, shuffle, shuffle::prover::shuffle_commit_product, vanishing, ChallengeBeta, ChallengeGamma, ChallengeTheta, ChallengeX, ChallengeY, Error, ProvingKey};
use crate::poly::{
...
use crate::multicore::{IntoParallelRefIterator, IntoParallelIterator, ParallelIterator};
#[cfg(feature = "mv-lookup")]
use crate::plonk::mv_lookup::prover::lookup_prepare;
    ...
    > Prover<'a, 'params, Scheme, P, E, R, T, M>
{
        ...
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
        ...
        let s = FxBuildHasher;
        let challenges = HashMap::<usize, Scheme::Scalar>::with_capacity_and_hasher(meta.num_challenges, s);
        ...
        Scheme::ParamsProver: Send + Sync,
    {
        ...
        let permuted_lookups: Vec<Vec<_>> = instances
            .iter()
            .zip(advices.iter())
        ...
        #[cfg(feature = "mv-lookup")]
        permuted_lookups.iter().for_each(|lookups| {
            lookups.iter().for_each(|lookup| {
                self.transcript.write_point(lookup.commitment);
            });
        });
        ...
        let lookups_commited: Vec<Vec<lookup::prover::Committed<Scheme::Curve>>> = permuted_lookups
            ...
            .map(|lookups| -> Result<Vec<_>, _> {
                ...
                lookups
                    .into_par_iter()
                    .map(|lookup| lookup.commit_grand_sum(&self.engine, &pk.vk, params, beta))
                    .collect::<Result<Vec<_>, _>>()
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        #[cfg(feature = "mv-lookup")]
        lookups_commited.iter().for_each(|lookups| {
            lookups.iter().for_each(|lookup| {
                self.transcript.write_point(lookup.commitment);
            });
        });
            ...
            lookups_commited
                ...
                .map(|lookups| -> Result<Vec<_>, _> {
                    lookups
                        ...
                        .map(|p| {
                            #[cfg(not(feature = "mv-lookup"))]
                            let res = { p.evaluate(pk, x, self.transcript) };
                            #[cfg(feature = "mv-lookup")]
                            let res = { p.evaluate(&pk.vk, x, self.transcript) };
                            res
                        })                        .collect::<Result<Vec<_>, _>>()
                })
                .collect::<Result<Vec<_>, _>>()?;
