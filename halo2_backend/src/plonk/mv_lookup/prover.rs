...
pub(in crate::plonk) fn lookup_prepare<'a, F, C, P: Params<C>, M>(
    ...
    M: MsmAccel<C>,
{
    ...
    let compressed_inputs_expressions: Vec<_> = arg
        .inputs_expressions
        .par_iter()
    ...
    let m_commitment = params.commit_lagrange(&engine.msm_backend, &m_values, blind).to_affine();
...
impl<C: CurveAffine> Prepared<C> {
    ...
    >(
        ...
        */
        ...
        let start = instant::Instant::now();
        let phi_commitment = params.commit_lagrange(&engine.msm_backend, &phi, grand_sum_blind).to_affine();
